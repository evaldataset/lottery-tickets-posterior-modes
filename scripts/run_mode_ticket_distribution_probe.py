...
from lottery.tempered_sgld import TemperedSGLDConfig, collect_tempered_sgld_samples
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Ascending comma-separated temperature ladder starting at 1.0",
    )
    parser.add_argument("--tempered-swap-every", type=int, default=10)
    ...
    parser.add_argument(
        "--activation-projection-dim",
        type=int,
        default=0,
        help=(
            "If > 0, apply a seeded Gaussian random projection to "
            "pre-classifier features wider than this before activation "
            "CKA (needed for TinyCNN-scale 4096-dim heads)."
        ),
    )
    parser.add_argument(
        "--alignment-method",
...
def feature_matrix(
    ...
    # Optional seeded Gaussian random projection for wide pre-classifier
    # features (e.g. TinyCNN's 4096-dim head input): linear CKA over O(n)
    # projected dimensions approximates full-feature CKA while keeping the
    # downstream 55x55 CKA matrix tractable. Built lazily on the first batch.
    ...
    def capture(_module: nn.Module, inputs: tuple[torch.Tensor, ...]) -> None:
        ...
        if projection_dim and features.shape[1] > projection_dim:
            if not projection:
                ...
                projection.append(matrix.to(features.device))
            features = features @ projection[0]
...
def sampler_config(args: argparse.Namespace, train_size: int):
    ...
    if args.posterior_sampler == "deep-ensemble":
        return {
            "members_per_chain": args.samples,
            "member_epochs": args.epochs,
            "num_train_examples": train_size,
        }
    if args.posterior_sampler == "tempered-sgld":
        return TemperedSGLDConfig(
            steps=steps,
            lr=args.sgld_lr,
            temperatures=tuple(
                float(t) for t in args.tempered_temperatures.split(",")
            ),
            prior_precision=args.sgld_prior_precision,
            burn_in=args.sgld_burn_in,
            sample_every=args.sgld_sample_every,
            swap_every=args.tempered_swap_every,
            num_train_examples=train_size,
            likelihood_scale=args.sgld_likelihood_scale,
            batchnorm_mode="eval" if args.posterior_bn_policy == "freeze" else "train",
        )
...
def sampler_metadata(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.posterior_sampler == "tempered-sgld":
        return {
            "steps": args.sgld_steps,
            "lr": args.sgld_lr,
            "temperatures": args.tempered_temperatures,
            "swap_every": args.tempered_swap_every,
            "prior_precision": args.sgld_prior_precision,
            "likelihood_scale": args.sgld_likelihood_scale,
            "burn_in": args.sgld_burn_in,
...
def collect_samples(
    ...
    if args.posterior_sampler == "tempered-sgld":
        tempered_samples, diagnostics = collect_tempered_sgld_samples(
            model, train_loader, device, config
        )
        print(
            json.dumps(
                {
                    "tempered_sgld_swap_attempts": diagnostics.swap_attempts,
                    "tempered_sgld_swap_accepts": diagnostics.swap_accepts,
                    "tempered_sgld_swap_acceptance_rate": diagnostics.swap_acceptance_rate,
                }
            ),
            flush=True,
        )
        return tempered_samples[: args.samples]
    if args.posterior_sampler == "sghmc":
    ...
    for seed in args.seeds:
        ...
        ticket_features.append(
            feature_matrix(
                ...
                eval_loader,
                device,
                projection_dim=args.activation_projection_dim,
                projection_seed=args.data_seed,
            ).numpy()
        )
        ...
        for chain_idx in range(args.posterior_chains):
            ...
            if args.posterior_sampler == "deep-ensemble":
                # Deep-ensemble "posterior samples": independently initialized
                # full trainings with the same recipe (the multimodal posterior
                # approximation of Wilson & Izmailov 2020), one fresh init per
                # member. The chain-start control stays the seed's own dense
                # net so the support-tracking comparison is unchanged.
                samples = []
                for member_idx in range(args.samples):
                    set_seed(seed + 30_000 + chain_idx * 1_000 + member_idx)
                    member_model = model_factory()
                    member_initial_state = state_to_cpu(member_model)
                    member_state, _member_rewind, _member_metrics, _member_history = (
                        train_dense_with_rewind_state(
                            model_factory,
