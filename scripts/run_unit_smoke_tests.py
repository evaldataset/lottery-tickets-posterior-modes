...
def check_tempered_sgld() -> dict[str, Any]:
    ...
    samples, diagnostics = collect_tempered_sgld_samples(
        model, loader, torch.device("cpu"), config
    )
    if len(samples) < 3:
        raise AssertionError(f"tempered SGLD returned too few samples: {len(samples)}")
    if diagnostics.swap_attempts <= 0:
        raise AssertionError("tempered SGLD attempted no swaps")
    for sample in samples:
        for tensor in sample.values():
            if not torch.isfinite(tensor).all():
                raise AssertionError("tempered SGLD sample contains non-finite values")
    bad_ladder = TemperedSGLDConfig(
        steps=1,
        lr=1e-4,
    ...
    try:
        collect_tempered_sgld_samples(model, loader, torch.device("cpu"), bad_ladder)
    except ValueError:
        pass
    else:
        raise AssertionError("tempered SGLD accepted a ladder not starting at 1.0")
    return {
        "name": "tempered_sgld",
        "passed": True,
        "covers": [
            "tempered_sgld_sample_collection",
            "tempered_sgld_swap_machinery",
            "tempered_sgld_ladder_validation",
        ],
    }
