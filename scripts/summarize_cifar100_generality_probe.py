#!/usr/bin/env python
"""Seed-level dataset-generality summary for the CIFAR-100 mode/ticket probe.

The full mode/ticket distribution probe runs the same per-seed pipeline on
CIFAR-100 ResNet-20 as it does on CIFAR-10. This summarizer aggregates the
*seed-level* outcome of that run --- dense chain-start accuracy, matched-sparsity
IMP accuracy, posterior SGLD sample accuracy, and the posterior-TopK-mask-to-
chain-start Hamming distance --- into a committed `runs/*.csv`/`runs/*.json`
pair plus a `docs/*.md` table.
...
reported alongside the regular ResNet-20 rows by
`summarize_mode_ticket_distribution_probe.py` and are not duplicated here: the
generality question is whether the seed-level failure pattern survives moving
the dataset, and that is a seed-level claim.
"""
...
DEFAULT_RUN_ROOT = (
    ROOT
    / "runs"
    / "cifar100_resnet20_long30_rewind1_mode_ticket_distribution_sgld_r5_p0p3"
)
DEFAULT_OUT_CSV = ROOT / "runs" / "cifar100_mode_ticket_generality_summary.csv"
DEFAULT_OUT_JSON = ROOT / "runs" / "cifar100_mode_ticket_generality.json"
DEFAULT_OUT_MD = ROOT / "docs" / "cifar100_mode_ticket_generality.md"
...
def write_markdown(path: Path, result: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        "",
        f"Dataset generality of the posterior-mode failure: **{status}**.",
        "",
