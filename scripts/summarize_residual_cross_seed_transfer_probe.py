...
def load_rows(run_root: Path) -> list[dict[str, object]]:
    ...
    for path in sorted(run_root.glob("*/metrics.json")):
        ...
        for row in payload["rows"]:
            ...
            rows.append(out)
    if not rows:
        raise SystemExit(f"no residual cross-seed transfer metrics found under {run_root}")
    return rows
...
def build_markdown(summary: list[dict[str, object]]) -> str:
    ...
    for base, variants in sorted(by_base.items()):
        cross = variants.get("cross_seed_predictor_residual")
        rand = variants.get("target_random_residual")
        oracle = variants.get("oracle_imp_residual")
        if cross is None or rand is None or oracle is None:
            continue
        lines.append(
            f"- {label(base)}: cross-seed Acc-IMP {fmt_ci(cross, 'accuracy_minus_imp')}, "
            f"random Acc-IMP {fmt_ci(rand, 'accuracy_minus_imp')}, "
            f"oracle Acc-IMP {fmt_ci(oracle, 'accuracy_minus_imp')}; "
            f"cross-seed added precision {fmt_ci(cross, 'added_imp_only_precision')} "
            f"versus random {fmt_ci(rand, 'added_imp_only_precision')}."
        )
    lines.append("")
    lines.append("This file is generated by `scripts/summarize_residual_cross_seed_transfer_probe.py`.")
    return "\n".join(lines)
