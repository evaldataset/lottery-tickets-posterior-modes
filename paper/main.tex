...
epoch-1 rewind setting, twelve posterior approximations---SGLD, SGHMC, cyclical
SGLD, SWAG, the Laplace family up to joint-group full covariance, and subspace
HMC---fail this stronger criterion. Posterior supports often beat random masks,
...
MNIST/Fashion-MNIST sweeps and twelve CIFAR-10 ResNet-20 posterior
...
support-equivalence gate, the twelve-approximation harness, the
...
signal is the IMP-selected residual, not a posterior-mode support. The cost is
quantitative: treating IMP tickets as posterior modes underperforms IMP by 2.3
...
addition to mask overlap. A central aim of this paper is to adjudicate between
these two competitors directly: the posterior-mode reading and the
axial-subspace/trajectory reading make opposite predictions about whether
posterior-induced supports beat trajectory and pruning-process controls, and
the operational gates below are designed to separate them.
...
are posterior modes.'' The \emph{strong} form (SPMH)---posterior-induced
support distributions coincide with the IMP ticket distribution in mask,
layer sparsity, and basin structure jointly---is already untenable at high
SNR by Proposition~\ref{prop:topk}, and rejecting it alone would be
trivial. The \emph{weak} form (WPMH), the substantive target of every gate
below, asserts only that posterior supports explain IMP tickets
\emph{beyond} the local magnitude, trajectory, and pruning-process controls
above. WPMH is the operational content of selection-view readings
\citep{ramanujan2020hidden, wilson2020bayesian}: a posterior support that
``reveals structure already present in the trained model'' must at minimum
carry ticket information the chain-start magnitude support does not.
...
Proposition~\ref{prop:topk} predicts the regime we observe. Trained weights are
high-SNR for the bulk of their large-magnitude coordinates, so the
posterior-sample TopK mask is forced to coincide with the chain-start mask
except at the keep/prune boundary---exactly the measured pattern in
Section~\ref{sec:posterior-fails}, where posterior-to-chain overlap stays near
0.91--0.98 and the posterior-minus-chain gap is indistinguishable from zero. It
also sharpens the burden of proof: a posterior account can differ from the
chain start only at low-SNR boundary coordinates, the regime where its own
samples are noisiest.

\paragraph{Why rank-bounded posteriors should struggle (informal).}\label{prop:rankcov}
Proposition~\ref{prop:topk} bounds marginal coordinate-level disagreement,
but the gate also tests \emph{joint} mask structure. A local Gaussian
...
coordinate-coordinate dependencies that are not Gaussian-latent. The
...
operational gates themselves (the proposal thresholds in
\texttt{scripts/run\_mode\_ticket\_distribution\_probe.py}) were fixed in
...
\begin{figure}[t]
\centering
\includegraphics[width=0.72\textwidth]{figures/gate1_controls.pdf}
...
posterior-to-chain-start overlap remains around 0.91--0.94. ``Near zero''
is equivalence-tested, not merely non-significant: a TOST reanalysis over
seed-level gaps bounds the gap within $\pm 0.005$ Jaccard on all eight
Gate1 rows. Dense magnitude is
...
The covariance objection is the strongest bounded limitation, so we push exact
covariance as far as the workstation budget permits, across five exact
block-diagonal and joint-group rows covering 22,064, 68,144, 68,144, 86,576,
and 270,896 trainable weights. Every row tells the same story. Sample accuracy
stays near 88\%, and global posterior-to-chain-start support falls as fidelity
grows (from 0.8287 at the 22,064-parameter row down to 0.7148--0.7863), so the
posterior does move. But it does not move toward tickets: the selected-block
...
reports global posterior-minus-chain $-0.0019$. These rows remove the practical
weight-tensor coverage objection, while literal dense CIFAR full-network
covariance remains outside budget: a single float64 dense matrix would require
553.1 GiB, and matrix plus Cholesky would require 1,106.3 GiB. Per-row numbers
are in the claim ledger.
...
same. A streamed exact joint-group direct probe over all 270,896 weight
parameters closes the full-weight Gaussian gap: 25 joint-group posterior
samples keep sample accuracy at 88.3\%, posterior-to-chain-start Hamming is
0.0503, layer-sparsity KS is $p=1.1{\times}10^{-8}$, Hamming overlap is 0.000,
logit/final-hidden activation CKA remains 0.937/0.920, and all 25 joint-group
samples still collapse to one parameter-PCA basin.
...
point) at the same five-round $p{=}0.30$ schedule and 0.832 sparsity. We report
\emph{only} the per-seed direction checks and the
posterior-to-chain-start Hamming for this cell; the joint-distribution gate
axes (posterior-versus-IMP Hamming, layer-sparsity KS, function-space CKA)
were not computed because the activation-CKA stage stalls on TinyCNN's wide
$4096$-dimensional pre-classifier feature. The seed-level direction is
unchanged: IMP beats the dense chain start in 5/5 seeds (0.791 versus 0.781);
SGLD posterior samples stay below both the dense chain start and the IMP
ticket in 5/5 seeds (0.738); and posterior TopK masks again track the chain
start with posterior-to-chain-start Hamming 0.061 (95\% CI $[0.060,0.062]$)
across seeds; the joint-distribution axes for a non-residual architecture
remain an open robustness item.
...
predictions that a posterior-mode account does not. (P+1) Same-seed IMP
masks should be \emph{closer to within-seed IMP ground truth} than masks
transferred from another seed, because the trajectory selects support per
seed. (P+2) Cross-seed transferred masks should still carry
\emph{above-random} support-prediction precision, because the
trajectory-magnitude component is partially shared across seeds that share
data, initialization scheme, and optimizer. Both predictions hold in the
existing five-seed cross-seed transfer probe: same-seed (oracle) accuracy
minus IMP is $-0.0090$ ($[-0.0111,\,-0.0068]$) while cross-seed accuracy
minus IMP is $-0.0235$ ($[-0.0269,\,-0.0201]$), a $1.5$-point per-seed
drop (P+1); and cross-seed added support precision is $0.2388$
($[0.2334,\,0.2442]$), roughly twice the random-mask baseline
$0.1264$ ($[0.1237,\,0.1292]$) at the same matched sparsity (P+2). A
posterior-mode account would predict shared seed-invariant support and
therefore little to no within/cross gap.
...
twelve posterior approximation configurations, and the validation-selected
...
matched-sparsity schedule. The claim
...
explicit multiple-comparisons exposure: every confirmatory row pools five
seeds against four to six proposal thresholds across twelve posterior
approximation configurations, so individual pooled p-values are descriptive
diagnostics rather than independent significance tests. A generated
family-wise audit quantifies the exposure over the fourteen distinct
full-data direct configurations ($14 \times 6 = 84$ trials): all fourteen
...
\begin{itemize}
  ...
  recalibrate, and dense-buffer policies leaves the negative direction
  intact in 5/6 rows; vanilla SGLD with frozen BN collapses to random
  predictions and is reported as an invalid sampler setting, so the result
  is not a BN-buffer artifact;
  ...
  pruning process;
  \item Reproducibility Statement. The repository has a dependency list,
...
Since the aligned, multi-chain, low-rank Laplace, block/joint-group Laplace,
locked final-test, and BatchNorm-policy CIFAR runs preserve the current
pattern, the next decisive empirical gap is an even stronger high-fidelity
posterior baseline; if it stays near the chain-start/rewind controls, the
negative result is the right paper, otherwise the claim should weaken to a
partial-equivalence statement (the rank-128 Hessian-plus-diagonal row passes
5/6 thresholds and is the strongest within-current-evidence example).
...
deployment claims. The main ethical risk is scientific overclaiming: readers
could misinterpret the negative result as a theorem against Bayesian accounts
of pruning rather than as a controlled empirical result under the tested
architectures, posterior approximations, sparsities, and compute budget. We
mitigate that risk by stating the operational support-equivalence criterion,
reporting negative and bounded findings, preserving open limitations, and
packaging code and artifacts for independent reproduction. Final submission
still requires the human authors to review and acknowledge the applicable ICLR
Code of Ethics.
...
\iflotteryincludeappendix
  ...
  twelve-approximation harness, pre-registered locked-test protocol, and
